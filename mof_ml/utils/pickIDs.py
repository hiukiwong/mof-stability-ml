...
def pick_centre_id(closest_metal_atoms_df) -> list:
    centre_atom = closest_metal_atoms_df.iloc[1]
    central_id = [closest_metal_atoms_df.index[1]]
    print(centre_atom)
...
def pick_xz_ids(z_ax_ids, z_axis_atoms, all_atoms_df):
    z_ax_id = z_ax_ids[0]
    z_ax_atom = z_axis_atoms.iloc[0]
    x_wanted = z_ax_atom.x
    y_wanted = z_ax_atom.y
    z_wanted = z_ax_atom.z
